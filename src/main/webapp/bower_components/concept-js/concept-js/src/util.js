...
Util.prototype.truncate = function(str, max) {
	var ending = '...';
    
    if (str.length > max) {
      return str.substring(0, max - ending.length) + ending;
    } else {
      return str;
  }
}
...
Util.prototype.displayCollection = function(parentDiv, collection, cloneProcessFun){
	$(collection).each(function(){
		var item = $(this)[0];
		var clone = $(parentDiv).find('.clone').clone();
		$(clone).removeClass('clone').removeClass('hide');
		cloneProcessFun(item, clone);
		$(parentDiv).append(clone);

		//divider
		var divider = $(parentDiv).find('.divider:first').clone();
...
Util.prototype.repeat =  function(element, collection, spacer){
	...
	$(collection).each(function(){
		...
		$(clone).find('.value').each(function(){
			...
			$(clone).find('.value[key="'+ key +'"]').html(elem[key]);
			$(clone).removeAttr('id');
			$(clone).fadeIn('normal');
		});
